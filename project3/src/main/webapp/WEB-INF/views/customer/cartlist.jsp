...
<script type="text/javascript">
    	function cartDelete(){
    		if(confirm("정말 삭제하시겠습니까?")){
    			...
    			window.location="cartDelete?chkArray="+chkArray;
    		}else{
    			return;
    		}
    	}
    	function buy(){
    			...
    			window.location="cartbuy_pro?chkArray="+chkArray;
    	}
    	$(function(){
...
<body>
    ...
    <div class="title">
	    		<h2 style="font-size:40px;">장바구니</h2>
	    	</div>
	    	<div class="titleli">	
		    		<ul>
		    			<li>홈 > </li>
	    	...
	    	<ul>
	    		...
	    		<li class="hit"><a href="${path}customer/cartlist" class="listyle">장바구니</a></li>
    ...
    <form action="${path}customer/cartbuy_pro" name="orderlistform" method="post">
	    <div class="cartlist" align="center">
			...
			<input type="hidden" name="CART_NOTB_ID" value="${dto.getCART_NOTB_ID()}">
	    	<input type="hidden" name="CU_ID" value="${sessionScope.memId}">
	    	<input type="hidden" name="CART_NOTB_NAME" value="${dto.getCART_NOTB_NAME()}">
	    	<input type="hidden" name="CART_NOTB_BRAND" value="${dto.getCART_NOTB_BRAND()}">
	    	<input type="hidden" name="CART_NOTB_IMG" value="${dto.getCART_NOTB_IMG()}">
	    	<input type="hidden" name="CART_NOTB_PRICE" value="${dto.getCART_NOTB_PRICE()}">
	    	<input type="hidden" name="CART_NOTB_CNT" value="${dto.getCART_NOTB_CNT()}">
				<tr>
					<td>
						<input type='checkbox' name="checkbox" value="${dto.getCART_NOTB_ID()}" onclick="checkAll();">
					</td>
					<td>
						<img src="${path}resources/images/${dto.getCART_NOTB_IMG()}" class="imgclass" >
					</td>
					<td>
						${dto.getCART_NOTB_NAME()}
					</td>
					<td>
						${dto.getCART_NOTB_BRAND()}
					</td>
					<td>
						${dto.getCART_NOTB_CNT()}
					</td>
					<td>
						<fmt:formatNumber value="${dto.getCART_NOTB_PRICE()}" pattern="#,###"/> 원
					</td>
				</tr>
			</c:forEach>
		</c:if>
		...
		<c:if test="${cnt == 0}">
		   ...
		   <div class="buttoncl" align="center">
	    	<div>
	    		<input type="button" id="button2"value="구매하기 " onclick="buy();">
	    		<input type="button" id="button3"value="장바구니삭제" onclick="cartDelete();">
	    	</div>
	    </div>
	    </form>
	   </div> 
